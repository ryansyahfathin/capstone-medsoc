...
if uploaded_file is not None:
    ...
    # Preprocessing
    df['Time'] = pd.to_datetime(df['Post Timestamp']).dt.time
    df['Date'] = pd.to_datetime(df['Post Timestamp']).dt.date
    df['Day'] = pd.to_datetime(df['Post Timestamp']).dt.day
    df['Month'] = pd.to_datetime(df['Post Timestamp']).dt.month
    df['Year'] = pd.to_datetime(df['Post Timestamp']).dt.year
    ...
    # Cari jam terbaik secara otomatis dari data
