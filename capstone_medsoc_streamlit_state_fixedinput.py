...
# Input menggunakan session state agar tidak trigger rerun output
if 'platform_input' not in st.session_state:
    st.session_state['platform_input'] = df['Platform'].unique()[0]
if 'post_type_input' not in st.session_state:
    st.session_state['post_type_input'] = df['Post Type'].unique()[0]
if 'gender_input' not in st.session_state:
    st.session_state['gender_input'] = df['Audience Gender'].unique()[0]

st.selectbox("Pilih Platform", df['Platform'].unique(), key="platform_input")
st.selectbox("Pilih Jenis Post", df['Post Type'].unique(), key="post_type_input")
st.selectbox("Pilih Gender Target", df['Audience Gender'].unique(), key="gender_input")

# Tombol prediksi trigger prediksi dari state
if st.button("Predict Engagement"):
    platform_input = st.session_state["platform_input"]
    post_type_input = st.session_state["post_type_input"]
    gender_input = st.session_state["gender_input"]

    # Buat input user menjadi dataframe dan encode sesuai format training
    data_input = {
